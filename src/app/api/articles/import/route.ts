...
import type { AppMetadata, ApiResponse, ArticleImportResult } from "@/lib/types";
 ...
 * Parses the file, validates rows, and upserts articles into the tenant's catalog.
 * Duplicate article_numbers within the same tenant are updated (upsert).
 */
export async function POST(
  request: NextRequest
): Promise<NextResponse<ApiResponse<ArticleImportResult>>> {
  try {
    ...
    return await processArticleImport(request, tenantId);
  } catch (error) {
    console.error("Unexpected error in POST /api/articles/import:", error);
    return NextResponse.json(
...
export async function processArticleImport(
  ...
  // Validate file size (max 10 MB for article imports)
    ...
    return NextResponse.json(
      {
        ...
        error: errors.length > 0
          ? `Keine gültigen Zeilen gefunden. ${errors[0]}`
          : "Keine gültigen Zeilen in der Datei gefunden.",
      },
      { status: 400 }
    );
  }
