...
import type { ApiResponse, OutputFormatParseResponse } from "@/lib/types";

const UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

/**
 * POST /api/admin/output-formats/[tenantId]/parse
 *
 * Accepts a sample file upload, parses it, and returns the detected schema
...
export async function POST(
  ...
  try {
    ...
    console.error("Error in POST /api/admin/output-formats/[tenantId]/parse:", error);
    return NextResponse.json(
